...
L.Control.GroupedLayers = L.Control.extend({ options: { collapsed: true, position: "topright", autoZIndex: true, exclusiveGroups: [], groupCheckboxes: false }, initialize: function(a, b, c) {
  ...
  for (d in b) for (e in b[d]) this._addLayer(b[d][e], e, d, true);
}, onAdd: function(a) {
  ...
  if (this._container) {
    ...
    for (var a, b, c = false, d = false, a = 0; a < this._layers.length; a++) b = this._layers[a], this._addItem(b), d = d || b.overlay, c = c || !b.overlay;
  ...
  for (this._handlingClick = true, a = 0; e > a; a++) b = d[a], "leaflet-control-layers-selector" === b.className && (c = this._getLayer(b.layerId), b.checked && !this._map.hasLayer(c.layer) ? this._map.addLayer(c.layer) : !b.checked && this._map.hasLayer(c.layer) && this._map.removeLayer(c.layer));
